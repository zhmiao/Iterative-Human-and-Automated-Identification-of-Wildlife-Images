...
class ModulatedAttLayer(nn.Module):
    ...
    def __init__(self, in_channels, feat_map_size=7, reduction=2, mode='embedded_gaussian'):
        ...
        self.avgpool = nn.AvgPool2d(self.feat_map_size, stride=1)
        self.fc_spatial = nn.Linear(self.feat_map_size * self.feat_map_size * self.in_channels,
                                    self.feat_map_size * self.feat_map_size)
    ...
    def embedded_gaussian(self, x):
        ...
        x_flatten = x.view(-1, self.feat_map_size * self.feat_map_size  * self.in_channels)

        spatial_att = self.fc_spatial(x_flatten)
        spatial_att = spatial_att.softmax(dim=1)

        spatial_att = spatial_att.view(-1, self.feat_map_size, self.feat_map_size).unsqueeze(1)
